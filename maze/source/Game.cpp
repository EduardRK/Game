...
Game::Game(Player &player, Maze &maze, std::vector<std::shared_ptr<Enemy>> &enemies, std::vector<std::shared_ptr<Item>> &items, Exit &exit) : _player{player}, _maze{maze}, _enemies{enemies}, _items{items}, _exit{exit}, _mazeRenderer{std::make_unique<GlutMazeRenderer>(player, maze)}
{
}
